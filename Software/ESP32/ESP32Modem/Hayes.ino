...
void EnterHayesMode()
{
  ...
  C64.println();
  C64.println(F("Hayes mode set.  Use AT&M to return to menu mode."));  
  HayesEmulationMode();
}
...
//  DSR, DTR, RI set here
  ...
  if (autoConnectHost >= 1 && autoConnectHost <= ADDR_HOST_ENTRIES)
  {
    ...
    C64.print(F("Auto-connecting to:\r\n"));
    C64.println(address);

    C64.println(F("\r\nPress any key to cancel..."));
    // Wait for user to cancel

    int option = PeekByte(Serial, 2000);

    if (option != TIMEDOUT)   // Key pressed
    {
      ReadByte(C64);    // eat character
      DisplayString(("OK"), true, 0);
      C64.print(F("OK"));
      C64.println();
    }
    else
    ...
    for (int i = 2; i < strlen(Modem_CommandBuffer) && i < COMMAND_BUFFER_SIZE - 3;)
    {
      ...
      case '&':   // AT&
        ...
        case 'M':   // AT&M
          mode_Hayes = false;
          updateEEPROMByte(ADDR_HAYES_MENU, mode_Hayes);
		  DisplayBoth("Restarting in Menu mode.");
          C64.println();
		  delay(1000);
          ESP.restart();
          while (1);
      ...
      if (Modem_isCommandMode)
      {
        ...
        */
        ...
        else if (inbound != '\r' && inbound != '\n')
        {
          ...
          {
            //DisplayString (F("CMD Buf Overflow"));
            Modem_PrintERROR();
            Modem_ResetCommandBuffer();
          }
          else 
